...
) 
{
    ...
    if (i >= H || j >= W) return;  // Out-of-bounds check
    ...
    // **Step 1: Cull Gaussians for this ray**
    __global__ int valid_indices[MAX_GAUSSIANS_PER_RAY];
    int valid_count = 0;  // Local counter for this thread

    for (int n = 0; n < N; ++n) {
        float3 mean = make_float3(means[n * 3], means[n * 3 + 1], means[n * 3 + 2]);
        float3 v = make_float3(mean.x - ro.x, mean.y - ro.y, mean.z - ro.z);

        bool in_front = (v.x * rd.x + v.y * rd.y + v.z * rd.z) > 0;
        ...
        float dist = sqrtf(cross_product.x * cross_product.x + 
                            cross_product.y * cross_product.y + 
                            cross_product.z * cross_product.z);

        if (!in_front || (dist > r_a[n] && dist > r_b[n])) continue;

        // Store the valid index
        if (valid_count < MAX_GAUSSIANS_PER_RAY) {
            valid_indices[valid_count] = n;
            valid_count++;
        }
    }
    ...
    for (int step = 0; step < max_steps; ++step) {
        ...
        // Loop through valid Gaussians
        for (int k = 0; k < valid_count; ++k) {
            int n = valid_indices[k]; 
...
std::tuple<torch::Tensor, torch::Tensor> sphere_trace_tensor(
    ...
    GSPLAT_CHECK_INPUT(origins);
    GSPLAT_CHECK_INPUT(directions);
    GSPLAT_CHECK_INPUT(r_a);
    GSPLAT_CHECK_INPUT(r_b);
    GSPLAT_CHECK_INPUT(axes_a);
    GSPLAT_CHECK_INPUT(axes_b);

    uint32_t H = origins.size(0);
    uint32_t W = origins.size(1);
    uint32_t N = means.size(0);

    auto total_distance = torch::zeros({H, W}, means.options());
    auto hit_color = torch::zeros({H, W, 3}, means.options());
